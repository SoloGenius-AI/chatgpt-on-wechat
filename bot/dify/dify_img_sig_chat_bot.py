...
class DifyBot(Bot):
    def __init__(self):
        super().__init__()
        self.sessions = DifySessionManager(DifySession, model=conf().get("model", const.DIFY_SIG))
    ...
    def reply(self, query, context: Context = None):
        ...
        if (context.type == ContextType.TEXT or context.type == ContextType.IMAGE_CREATE or
            ...
            logger.info("[DIFY_SIG] query={}".format(query))
    ...
    def _reply(self, query: str, session: DifySession, context: Context):
        try:
            ...
            error_info = f"[DIFY_SIG] Exception: {e}"
    ...
    def _handle_chatbot(self, query: str, session: DifySession, context: Context):
        ...
        if context.type != ContextType.IMAGE:
            ...
            logger.info(f'[DIFY_SIG] send {payload=}')
            ...
            try:
                ...
                logger.error(f'[DIFY_SIG] 图片格式转换失败: {save_file_path_}, err: {e_}')
            ...
            try:
                ...
                logger.info(f'[DIFY_SIG] remove {save_file_path_} failed. err: {e_}')
        if response.status_code != 200 and response.status_code != 201:
            error_info = f"[DIFY_SIG] response text={response.text} status_code={response.status_code}"
        ...
        logger.debug("[DIFY_SIG] usage {}".format(rsp_data.get('metadata', {}).get('usage', 0)))
        # TODO: 处理返回的图片文件
        ...
        if context.type != ContextType.IMAGE:
            ...
            try:
                ...
                logger.info(f'[DIFY_SIG] reply添加add_info_dict异常: {e_}')
            ...
            image_id = rsp_data.get('id', None)
            logger.info(f'[DIFY_SIG] 图片上传成功, image_id: {image_id}')
            payload['inputs']['ask_image'] = 'true'
            payload['files'] = [{'type': 'image', 'transfer_method': 'local_file', 'upload_file_id': image_id}]
            payload['conversation_id'] = ''
            logger.info(f'[DIFY_SIG] 图片payload: {payload}')
            response = requests.post(chat_url, headers=headers, json=payload)
            if response.status_code != 200 and response.status_code != 201:
                error_info = f"[DIFY_SIG] response text={response.text} status_code={response.status_code}"
                logger.warn(error_info)
    ...
    def _handle_sse_response(self, response: requests.Response):
        ...
        for event in events:
            ...
            if event_name == 'agent_message' or event_name == 'message':
                ...
                logger.debug("[DIFY_SIG] accumulated_agent_message: {}".format(accumulated_agent_message))
                # 保存conversation_id
                ...
                accumulated_agent_message = ''
                logger.debug("[DIFY_SIG] agent_thought: {}".format(event))
                ...
                logger.error("[DIFY_SIG] error: {}".format(event))
                raise Exception(event)
            elif event_name == 'message_end':
                self._append_agent_message(accumulated_agent_message, merged_message)
                logger.debug("[DIFY_SIG] message_end usage: {}".format(event['metadata']['usage']))
                break
            else:
                logger.warn("[DIFY_SIG] unknown event: {}".format(event))
    ...
    def _append_message_file(self, event: dict, merged_message: list):
        if event.get('type') != 'image':
            logger.warn("[DIFY_SIG] unsupported message file type: {}".format(event))
        merged_message.append({
            'type': 'message_file',
            'content': event,
        })